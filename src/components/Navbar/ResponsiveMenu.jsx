...
>
  ...
  <div className="flex items-center justify-between px-6 py-4 border-b border-yellow-300 dark:border-amber-600 bg-yellow-50 dark:bg-gray-800">
    <div className="flex items-center space-x-3">
      ...
      <div>
        <span className="font-bold text-gray-800 dark:text-white">Dhyana Cab</span>
        <div className="text-xs text-yellow-600 dark:text-yellow-400 font-medium">Menu</div>
      </div>
    </div>
    <button
      onClick={closeMenu}
      ...
      {Navlinks.map((data, index) => (
        ...
        >
          <Link 
            ...
            onClick={closeMenu}
          >
            <span className="font-medium text-lg relative z-10">{data.name}</span>
            <svg 
